...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  untappd-pp-cli doctor
  untappd-pp-cli doctor --json
  untappd-pp-cli doctor --fail-on warn
  untappd-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					reachBody, reachErr := c.GetWithHeaders(cmd.Context(), healthPath, nil, htmlHeaders())
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "untappd-pp-cli", suggestion)
						} else {
