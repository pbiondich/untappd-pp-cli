...
type rootFlags struct {
	...
	// invocation. Mirrors the UNTAPPD_NO_LEARN env var.
	noLearn                 bool
	selectFields            string
	configPath              string
	homePath                string
...
func Execute() (retErr error) {
	...
	if rewritten := RewriteUserFacingArgs(os.Args); !argsEqual(rewritten, os.Args) {
		rootCmd.SetArgs(rewritten[1:])
	}
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "untappd-pp-cli",
		Short: `Untappd isn't just a check-in social network. It's a public beer intelligence layer: global ratings, style, ABV, IBU, and check-in volume that agents can query without the closed official API.`,
		Long: `Untappd isn't just a check-in social network. It's a public beer intelligence layer: global ratings, style, ABV, IBU, and check-in volume that agents can query without the closed official API.

Add --agent to any command for JSON output + non-interactive mode.
Run 'untappd-pp-cli doctor' to verify connectivity.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("untappd-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'untappd-pp-cli profile list')")
	...
	rootCmd.AddCommand(newBeerCmd(flags))
	rootCmd.AddCommand(newBreweryCmd(flags))
