...
func newLearnConfig() *entities.Config {
	...
	if re, err := regexp.Compile(`^[0-9]{3,8}$`); err != nil {
		fmt.Fprintf(os.Stderr, "warning: untappd-pp-cli: ticker pattern %q failed to compile: %v\n", `^[0-9]{3,8}$`, err)
	} else {
...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"brewery": {
			{Canonical: "Hop Butcher For The World", Values: []string{"hop butcher", "hopbutcher", "south loop brewing"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: untappd-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: untappd-pp-cli: learn init: %v\n", err)
		}
	})
}
