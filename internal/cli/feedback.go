...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("UNTAPPD_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("UNTAPPD_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "untappd-pp-cli/feedback")
...
When ` + "`UNTAPPD_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`UNTAPPD_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		Example: `  untappd-pp-cli feedback "the --since flag is inclusive but docs say exclusive"
  untappd-pp-cli feedback --stdin < notes.txt
  untappd-pp-cli feedback list --limit 10`,
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  untappd-pp-cli feedback list
  untappd-pp-cli feedback list --limit 5
  untappd-pp-cli feedback list --json`,
